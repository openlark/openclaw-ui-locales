...
1. 在 git 仓库内将`./extensions/feishu`目录复制到 `~/.openclaw/extensions/feishu`。
2. 确保包含 `index.ts`, `openclaw.plugin.json` 和 `package.json`。
3. 如果有依赖包,在`~/.openclaw/extensions/feishu`目录下运行 `npm install`。
4. 运行 `openclaw plugins list` 确认 `feishu` 已显示在列表中。
...
![创建企业自建应用](/assets/feishu-step2-create-app.png)
...
![获取应用凭证](/assets/feishu-step3-credentials.png)
...
![配置应用权限](/assets/feishu-step4-permissions.png)
![批量导入](/assets/feishu-step4-permissions-batchimport.png)
...
![启用机器人能力](/assets/feishu-step5-bot-capability.png)
...
在 **事件订阅** 页面：
![配置事件订阅](/assets/feishu-step6-event-subscription.png)

1. 选择 **使用长连接接收事件**（WebSocket 模式）
2. 保存之后，再添加事件：`im.message.receive_v1`（接收消息）

⚠️ **注意**：如果网关未启动或渠道未添加，长连接设置将保存失败。

![配置事件订阅](/assets/feishu-step6-event-subscription-add.png)
...
运行以下命令，选中飞书通道，根据提示粘贴 App ID 和 App Secret：

```bash
openclaw channels add
```

或者直接运行以下命令添加飞书App ID 和 App Secret：
...
完整配置请参考：[网关配置](https://docs.openclaw.ai/zh-CN/gateway/configuration)
